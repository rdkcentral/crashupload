...
int main_test(int argc, char *argv[])
{
    ...
    CRASHUPLOAD_INFO("After scan dump found:%d\n", dump_count);
    ...
    {
        CRASHUPLOAD_INFO("List of dump file=%s\n", (dumps + i)->path);
        process_file_entry((dumps + i)->path, argv[2], &config);
        CRASHUPLOAD_INFO("List of dump file After process_file_entry=%s\n", (dumps + i)->path);
        len = strlen((dumps + i)->path);
        if (len > 4 && strcmp((dumps + i)->path + len - 4, ".tgz") == 0)
        {
            CRASHUPLOAD_INFO("Skip archiving %s as it is a tarball already.\n", (dumps + i)->path);
        ...
        if (is_process_dmp_file == false)
        {
            if (config.dump_type == DUMP_TYPE_COREDUMP)
            {
                if (NULL != (strstr((dumps + i)->path, "mpeos-main")))
                {
                    ...
                    CRASHUPLOAD_INFO("new dump name crated for mpeos-main=%s\n", new_dump_name);
                }
                else
                {
                    ...
                    CRASHUPLOAD_INFO("new dump name crated for core dump=%s\n", new_dump_name);
                }
            }
            else
            {
                snprintf(new_dump_name, sizeof(new_dump_name), "%s_mac%s_dat%s_box%s_mod%s_%s", platform.platform_sha1, platform.mac_address, crashts, config.box_type, platform.model, dump_file_name);
                CRASHUPLOAD_INFO("new dump name crated for mini dump=%s\n", new_dump_name);
            }
        }
        else
        {
            CRASHUPLOAD_INFO("Core name is already processed.%s\n", dump_file_name);
        ...
        if (strlen(new_dump_name) >= 135)
        {
            ...
            if (tmp != NULL)
            {
                ...
                CRASHUPLOAD_INFO("After stripping dump file=%s\n", new_dump_name);
            }
        }
        if (strlen(new_dump_name) >= 135)
        {
            ...
            if (!pname)
            {
                CRASHUPLOAD_INFO("process name not found to do trim\n");
            }
            else
            {
                CRASHUPLOAD_INFO("Change the process name process name length to 20 byte-%s\n", pname);
                trim_process_name_in_path(new_dump_name, pname, 20, trim_dump_name, sizeof(trim_dump_name));
                CRASHUPLOAD_INFO("Changed File Name : %s\n", trim_dump_name);
                strncpy(new_dump_name, trim_dump_name, sizeof(new_dump_name));
            }
        }
        CRASHUPLOAD_INFO("Processing for TAR:%s\n", new_dump_name);
    ...
    {
        CRASHUPLOAD_INFO("Rate Limit is blocked. Exit\n");
    ...
    for (int i = 0; i < dump_count; i++)
    {
        ...
        {
            CRASHUPLOAD_INFO("Coredump File :%s\n", archive[i].archive_name);
        }
        ret = upload_process(&archive[i], &config, &platform);
        CRASHUPLOAD_INFO("upload_process return ret=%d\n", ret);
        if (ret != 0)
        {
            break;
        }
