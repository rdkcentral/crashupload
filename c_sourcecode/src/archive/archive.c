...
/* --------------------------------------------------------- */
static int archive_add_file(struct archive *a, const char *path)
{
    ...
    CRASHUPLOAD_INFO("archive_add_file: path=%s size=%ld mtime=%ld\n", path, (long)st.st_size, (long)st.st_mtime);
    ...
    CRASHUPLOAD_INFO("archive_add_file: writing header for %s\n", path);
    ...
    CRASHUPLOAD_INFO("archive_add_file: completed writing %s\n", path);
    ret_result = 0;
                          ...
                          size_t count)
{
    ...
    {
        CRASHUPLOAD_INFO("Setting Low Priority\n");
        set_low_priority();
    }
                                 ...
                                 char *filename, char *process_log_file, size_t size)
{
    ...
    {
        CRASHUPLOAD_ERROR("add_crashed_process_log_file() Invalid argument\n");
        return ret;
    }
    ...
    {
        CRASHUPLOAD_INFO("mtime of file:%s is %s\n", filename, mtime_date);
    }
    else
    {
        CRASHUPLOAD_WARN("mtime of file:%s is error\n", filename);
    }
    ...
    CRASHUPLOAD_INFO("File copy from:%s=>%s\n", filename, process_log_file);
                         ...
                         archive_info_t *archive, char *new_dump_name)
{
    ...
    {
        CRASHUPLOAD_ERROR("archive_create_smart Invalid Argument\n");
        return -1;
    }

    const char *pattern = "<#=#>";
    const size_t pattern_len = 5; // Length of "<#=#>"
    CRASHUPLOAD_INFO("archive_create_smart() Before process dump name=%s\n", new_dump_name);
    ...
    if (rename(dump->path, new_dump_name) != 0)
    {
        if (errno == EEXIST || errno == EACCES)
        {
            ...
            if (rename(dump->path, new_dump_name) != 0)
            {
                CRASHUPLOAD_ERROR("Failed to rename %s -> %s : %s", dump->path, new_dump_name, strerror(errno));
                return -1;
            }
            else
            {
    ...
    {
        CRASHUPLOAD_INFO("Size of the file:%llu\n", (unsigned long long)size_dump_u64);
    }
    ...
    else
    {
        ...
        if (config->device_type == DEVICE_TYPE_MEDIACLIENT)
        {
            ...
            if (0 == (filePresentCheck(crash_url_file)))
            { // TODO: Add logic if file has data
            ...
            if (fp != NULL)
            {
                while (fgets(buf, sizeof(buf), fp))
                {
                    ...
                    if (!add_crashed_process_log_file(config, platform, buf, process_name_log, sizeof(process_name_log)))
                    {
                        ...
                        CRASHUPLOAD_INFO("process log file=%s\n", arch_files_list[no_of_files]);
                        no_of_files++;
                        cnt_process_log_file++;
                    }
                    else
                    {
                        CRASHUPLOAD_ERROR("Error to add add_crashed_process_log_file():%s\n", buf);
                    }
                }
            ...
            {
                CRASHUPLOAD_ERROR("Error in opening file:%s\n", LOG_FILES_PATH);
            }
            ...
            CRASHUPLOAD_INFO("Before tar list of file name:%s\n", target_file_name);
            CRASHUPLOAD_INFO("dump file=%s\n", arch_files_list[0]);
            CRASHUPLOAD_INFO("version file=%s\n", arch_files_list[1]);
            CRASHUPLOAD_INFO("core log file=%s\n", arch_files_list[2]);
            tar_status = create_tarball(true, target_file_name, arch_files_list, no_of_files);
    ...
    {
        CRASHUPLOAD_ERROR("TAR creation failed try by coping to /tmp\n");
