...
int read_RFCProperty(char *type, const char *key, char *out_value, size_t datasize)
{
    ...
    if (status == WDMP_SUCCESS || status == WDMP_ERR_DEFAULT_VALUE)
    {
        ...
        CRASHUPLOAD_INFO("read_RFCProperty() name=%s,type=%d,value=%s,status=%d\n", param.name, param.type, param.value, status);
        ret = READ_RFC_SUCCESS;
    }
    else
    {
        CRASHUPLOAD_ERROR("error:read_RFCProperty(): status= %d\n", status);
        *out_value = 0;
    }
    return ret;
}
...
int write_RFCProperty(char *type, const char *key, const char *value, RFCVALDATATYPE datatype)
{
    ...
    {
        CRASHUPLOAD_ERROR("%s: Parameter is NULL\n", __FUNCTION__);
        return ret;
    }
    ...
    {
        CRASHUPLOAD_ERROR("%s: setRFCParameter failed. key=%s and status=%s\n", __FUNCTION__, key, getRFCErrorString(status));
    }
    else
    {
        CRASHUPLOAD_INFO("%s: setRFCParameter Success\n", __FUNCTION__);
...
int read_RFCProperty(const char *type, const char *key, char *out_value, size_t datasize)
{
    ...
    (void)out_value;
    (void)datasize;
    CRASHUPLOAD_INFO("%s: Not Applicable For RDK-M. Need to implement\n", __FUNCTION__);
