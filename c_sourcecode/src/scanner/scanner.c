...
int sanitize_filename_preserve_container(const char *fname, char *out, size_t outsz)
{
    ...
    while ((match = strstr(p, DELIM)) != NULL)
    {
        ...
        char tmp[128];
        if (seg_len >= sizeof(tmp))
            seg_len = sizeof(tmp) - 1;
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        *eq = '\0';
        ...
        if (strstr(pname, lhs) != NULL)
        {
            ...
            if (result != NULL)
            {
                CRASHUPLOAD_INFO("strdup success\n");
                CRASHUPLOAD_INFO("result = %s\n", result);
            }
            else
            {
                CRASHUPLOAD_ERROR("strdup not success\n");
            }
            break;
        }
    }
...
STATIC_TESTABLE int get_crashed_log_file(const char *file, const char *log_path, bool t2_enabled)
{
    ...
    CRASHUPLOAD_INFO("Process crashed = %s\n", pname);
    if (t2_enabled)
    {
    ...
    if (logrhs)
    {
        ...
        CRASHUPLOAD_INFO("Crashed process log file(s): %s\n", logrhs);
        /* Split comma-separated list and append LOG_PATH/<name> to LOG_FILES_PATH */
        char *saveptr = NULL;
        token = strtok_r(logrhs, ",", &saveptr);
        while (token != NULL)
        {
            CRASHUPLOAD_INFO("Inside Toke(s)\n");
            CRASHUPLOAD_INFO("token=%s\n", token);
            /* Trim leading/trailing spaces */
            while (*token && isspace((unsigned char)*token))
                token++;
            CRASHUPLOAD_INFO("Inside Toke1(s)\n");
            ...
            *end = '\0';
            ...
            if (join_path(logfull, sizeof(logfull), log_path, token) == 0)
            {
                ...
                {
                    CRASHUPLOAD_ERROR("Failed to append log entry: %s\n", logfull);
                }
            }
            else
            {
                CRASHUPLOAD_ERROR("Path too long for log: %s/%s", LOG_FILES_PATH, token);
            }
            *end = tmp;
            token = strtok_r(NULL, ",", &saveptr);
        }
        CRASHUPLOAD_INFO("Going to free logrhs\n");
        free(logrhs);
    }
    else
    {
        CRASHUPLOAD_INFO("No log mapper entry found for process\n");
    }

    if (appname)
    {
        CRASHUPLOAD_INFO("Appname, Process_Crashed = %s, %s\n", appname, pname);
        free(appname);
    }
...
int processCrashTelemetryInfo(const char *rawfile, const char *log_path, bool t2_enabled)
{
    ...
    /* Remove leading "./" */
    if (strncmp(file, "./", 2) == 0)
        memmove(file, file + 2, strlen(file + 2) + 1);
    ...
    if (ext && strcmp(ext, ".tgz") == 0)
    {
        ...
        if (pmod)
        {
            ...
            CRASHUPLOAD_INFO("Removed the meta information from tgz filename\n");
            CRASHUPLOAD_INFO("tgz file name after remove meta info=%s\n", file);
    ...
    if (strstr(file, containerDelimiter) != NULL)
    {
        ...
        CRASHUPLOAD_INFO("Container crash info Basic: %s, %s\n", Appname, ProcessName);
        CRASHUPLOAD_INFO("Container crash info Advance: %s, %s\n", containerName, containerStatus);
        CRASHUPLOAD_INFO("NEW Appname, Process_Crashed, Status = %s, %s, %s\n", Appname, ProcessName, containerStatus);
        CRASHUPLOAD_INFO("NEW Processname, App Name, AppState = %s, %s, %s\n", ProcessName, Appname, containerStatus);
...
int process_file_entry(char *fullpath, char *dump_type, const config_t *config)
{
    ...
    {
        CRASHUPLOAD_INFO("process_file_entry():Invalid Parameter\n");
        return -1;
    }
    CRASHUPLOAD_INFO("process_file_entry(): Full path:%s\n", fullpath);
    /* Only process regular files */
    if (!is_regular_file(fullpath))
    {
        CRASHUPLOAD_INFO("process_file_entry(): Not a regular file. return 0\n");
        return 0;
    }
    ...
    {
        CRASHUPLOAD_INFO("process_file_entry(): File name is cross max length:%d\n", PATH_MAX_LEN);
        return -1;
    }
    strncpy(fullcopy, fullpath, PATH_MAX_LEN);
    fullcopy[PATH_MAX_LEN - 1] = '\0';

    CRASHUPLOAD_INFO("full path of the file=%s=>\n", fullpath);
    ...
    CRASHUPLOAD_INFO("process_file_entry dir name=%s=>\nbasename=%s=>\n", dirname, basename);
    ...
    if ((sanitize_ret == 0) && (sanitized[0] == '\0'))
    {
        ...
        {
            CRASHUPLOAD_ERROR("Failed to unlink (sanitized empty): %s : %s\n", fullpath, strerror(errno));
            return -1;
        }
        CRASHUPLOAD_ERROR("Removed file with empty sanitized name: %s\n", fullpath);
        return 0;
    }
    ...
    if (strcmp(sanitized, basename) != 0)
    {
        ...
        {
            CRASHUPLOAD_INFO("Path too long after sanitization; skipping rename\n");
            return -1;
        }
        ...
        if (rename(fullpath, newfull) != 0)
        {
            ...
            if (errno == EEXIST || errno == EACCES)
            {
                ...
                if (rename(fullpath, newfull) != 0)
                {
                    CRASHUPLOAD_ERROR("Failed to rename %s -> %s : %s", fullpath, newfull, strerror(errno));
                    return -1;
                }
                else
                {
                    CRASHUPLOAD_INFO("Renamed After retry %s -> %s\n", fullpath, newfull);
                    strcpy(fullpath, newfull);
                    CRASHUPLOAD_INFO("After rename fullpath buffer updated=%s=\n", fullpath);
                }
            }
            else
            {
                CRASHUPLOAD_ERROR("Failed to rename %s -> %s : %s\n", fullpath, newfull, strerror(errno));
                return -1;
            }
        }
        else
        {
            CRASHUPLOAD_INFO("Renamed %s -> %s\n", fullpath, newfull);
            strcpy(fullpath, newfull);
            CRASHUPLOAD_INFO("After rename fullpath buffer updated=%s=\n", fullpath);
        }
        ...
        {
            CRASHUPLOAD_WARN("processCrashTelemetryInfo is not allowed\n");
        }
    }
    else
    {
...
STATIC_TESTABLE int is_dump_file(const char *filename, const char *dumps_extn_pattern)
{
    ...
    else
    {
        ...
        if (match == 1)
        {
            CRASHUPLOAD_INFO("core name match=%s\n", filename);
            return 2;
        }
        else
        {
            CRASHUPLOAD_INFO("core name not match=%s\n", filename);
            return 0;
        }
    }
...
int scanner_find_dumps(const char *path, dump_file_t **dumps, int *count, const char *dumps_extn_pattern)
{
    ...
    while ((entry = readdir(dir)) != NULL && dump_count < MAX_DUMPS)
    {
        ...
        CRASHUPLOAD_INFO("Checking if file writing is complete: %s\n", fullpath);
        /* Parameters: check every 1 sec, need 2 stable readings, max 10 iterations */
        if (wait_for_file_size_stable(fullpath, 1, 2, 10) != 0)
        {
            CRASHUPLOAD_INFO("Skipping file (still being written or error): %s\n", fullpath);
            continue;
        }
        CRASHUPLOAD_INFO("File writing complete: %s\n", fullpath);
        ...
        CRASHUPLOAD_INFO("Dump/Core file name=%s\n", found_dumps[dump_count].path);
        dump_count++;
    }
